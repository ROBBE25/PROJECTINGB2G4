...
#include <math.h> // Toegevoegd voor log10-functie
...
loop()
{
  int32_t value;
  char output[256];
  const float Vr =1; //referentie waarde die bij welke volt we 0 decibel hebben
  ...
  if (rx_buf_flag) {
    for (int x = 0; x < SLC_BUF_LEN; x++) {
      if (i2s_slc_buf_pntr[rx_buf_idx][x] > 0) {
        ...
        float dB = 20.0 * log10(abs((float)value) / Vr);

        sprintf(output, "%.2f dB", dB);
